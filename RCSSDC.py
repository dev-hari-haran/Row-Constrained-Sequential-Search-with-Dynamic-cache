...
class GridBotGUI(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Row-Priority Search (Hard-Constrained)")
    ...
    def init_ui(self):
        ...
        title = QLabel("🤖 Database-Guided Row Search (Hard Constraints)")
    ...
    def build_grids(self):
        ...
        self.cache = self.make_grid("Cache (Visited)")
    ...
    def populate_database(self):
        ...
        for r in range(self.grid_size):
            for c in range(self.grid_size):
                self.db["table"].setItem(r, c, QTableWidgetItem(str(val)))
                val += 1
        ...
        for r in range(self.grid_size):
            for c in range(self.grid_size):
                self.world["table"].setItem(
                    r, c, QTableWidgetItem(self.db["table"].item(r, c).text())
                )
    ...
    def randomize_world(self):
        ...
        for r in range(self.grid_size):
            for c in range(self.grid_size):
                self.world["table"].setItem(r, c, QTableWidgetItem(vals[i]))
                i += 1
    ...
    def build_search_rows(self, r, n):
        rows = []

        def add(row):
            if 0 <= row < n and row not in rows:
                rows.append(row)
    ...
    def start(self):
        ...
        self.search_rows = self.build_search_rows(base_row, self.grid_size)
    ...
    def step(self):
        ...
        # Move strictly left → right within a row
        self.current_col += 1
        if self.current_col >= self.grid_size:
            self.current_row_index += 1
            self.current_col = 0
            self.dir = 1   # reset direction per row (critical fix)
